document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ================= ADMIN EXCEL DOWNLOAD — STRICT FORMAT ================= */
  $("downloadExcelBtn").addEventListener("click", async () => {
    ...
    // HEADER ROW 1 — EXACT GROUP HEADER MATCH
    const headerRow1 = [
      "PERSONAL","","","","","","","","","",
      "OLEVEL","","",
      "ALEVEL",
    ...
    // HEADER ROW 2 — EXACT FIELD HEADER MATCH
    const headerRow2 = [
      "SNO","SURNAME","FIRSTNAME","OTHERNAMES","CADRE","GENDER",
      "BLOOD GROUP","STATE","LGA","DATE OF BIRTH",
      "TYPE","YEAR","EXAM NUMBER",
      "PROFESSIONAL CERTIFICATE NUMBER",
      "ENG","MATHS","BIO/HS","CHEMISTRY","PHYSIC",
      "REMARKS"
    ];

    const sheet = [headerRow1, headerRow2];

    data.forEach((r, i) => {
      const [eG] = split(r.ENGLISH);
      const [mG] = split(r.MATHEMATICS);
      const [bG] = split(r.BIOLOGY);
      const [cG] = split(r.CHEMISTRY);
      const [pG] = split(r.PHYSICS);
    ...
    XLSX.utils.book_append_sheet(wb, ws, "INDEXING DETAILS");
    XLSX.writeFile(wb, "INDEXING_DETAILS_EXACT_HEADER.xlsx");
  });
});